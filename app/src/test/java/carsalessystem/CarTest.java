...
class CarTest {
    ...
    void setUp() {
        ...
        car = new Car("sd24 SJD", "Ford", "Focus ", 50000, 2015, "Blue", "Air con and usb ports");
    }
    ...
    void testStaticCarCount() {
        ...
        assertEquals(0, Car.getCarCount());  // Initially should be 0
        new Car("XY987AB", "Honda", "Civic", 30000, 2020, "Red", "Sunroof");
        assertEquals(1, Car.getCarCount());  // After creating a car, car count should be 1
        Car.setCarCount(5);  // Manually setting car count to 5
        assertEquals(6, Car.getCarCount());  // Should now be 6 (5 + 1)
    }
    ...
    void testSetCarAgeWithInvalidYear() {
        ...
        car.setCarAge(1900);
        assertEquals(124, car.getCarAge());  // 2024 - 1900 = 124 (invalid year should be treated as car age)
    }
