    ...
    public void viewFinanceRecord(){
        if(!carCashPriceList.isEmpty()){
            for (CanBeSold cBS : carCashPriceList)
            {
                ...
                //use this to add the display finance info on option 8
                for (CanBeFinanced canBeFinanced : carFinanceList) {
                    if (cBS.getCarReg().equals(canBeFinanced.getCarReg()) && (cBS.getCashPrice() == canBeFinanced.getCashPrice())) {
                        System.out.print("\tFinance Duration: " + canBeFinanced.getFinanceDuration() + " months");
                        break;
                    }
                }
                System.out.println();
